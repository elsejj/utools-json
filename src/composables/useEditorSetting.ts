...
export function loadSetting(): EditorSetting {
  ...
  if (raw) {
    try {
      ...
      if (s.nativeParser === undefined || s.nativeParser === null) {
        s.nativeParser = false;
      }
  ...
  return {
    fontFamily:
      "'Cascadia Code NF', 'JetBrains Mono','Fira Code Retina', Consolas, 'Source Code Pro', 'Menlo', 'Courier New', monospace, 'Source Han Sans VF', '思源黑体'",
    fontSize: 13,
    theme: "",
    sortKey: true,
    aiModel: "",
