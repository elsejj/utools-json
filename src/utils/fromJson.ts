...
export function tomlFromJson(jsonText: string): string {
  const jsonObject = JSON5.parse(jsonText);
  return TOML.stringify(jsonObject);
}
...
<head>
<meta http-equiv=Cont@t-Type content="text/html; charset=utf-8">
...
export function htmlPlainFromJson(jsonText: string): string {
  ...
  const lines = [HTML_PREFIX, '<table class="json_table_v000">'];
  lines.push("<tr>");
  ...
  for (let row = 0; row < table.cells.length; row++) {
    ...
    for (let col = 0; col < table.cells[row].length; col++) {
      ...
      let rowSpan = 1;
      while (
        row + rowSpan < table.cells.length &&
        table.cells[row + rowSpan][col] === null
      ) {
        rowSpan++;
      }
      if (rowSpan > 1) {
        lines.push(
          `<td rowspan="${rowSpan}" class="${cellClass}">${cell}</td>`
        );
      } else {
...
export function htmlNestedFromJson(jsonText: string): string {
  ...
  for (let row = 0; row < nestedHeaders.length; row++) {
    ...
    for (let col = 0; col < nestedHeaders[row].length; ) {
      let rowSpan = 1;
      while (
        row + rowSpan < nestedHeaders.length &&
        nestedHeaders[row + rowSpan][col] === ""
      ) {
        rowSpan++;
      }

      let colSpan = 1;
      let lastNotSame = headers.findIndex(
        (header, i) => i > col && header !== headers[col]
      );
      if (lastNotSame === -1) {
