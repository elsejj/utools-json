...
async function replacePaste() {
  ...
  navigator.clipboard.readText().then((text) => {
    ...
    const jsonBody = new StringToJSON(
      setting.value.sortKey,
      setting.value.nativeParser,
    ).toJSON(text);
    if (jsonBody) {
...
onMounted(() => {
  if (monacoNode.value) {
    ...
    diffEditor
      .getOriginalEditor()
      .onDidPaste(onDidPaste.bind(diffEditor.getOriginalEditor()));
    diffEditor
      .getModifiedEditor()
      .onDidPaste(onDidPaste.bind(diffEditor.getModifiedEditor()));
...
onUnmounted(() => {
  if (diffEditor) {
    ...
    diffEditor
      .getOriginalEditor()
      .trigger("editor", "editor.action.fontZoomReset", {});
    diffEditor
      .getModifiedEditor()
      .trigger("editor", "editor.action.fontZoomReset", {});
